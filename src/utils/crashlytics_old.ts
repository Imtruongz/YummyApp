...
export const setUserContext = (userId: string, additionalAttributes?: Record<string, string>): void => {
  try {
    ...
    import { setUserId } from '@react-native-firebase/crashlytics/lib/modular';
    setUserId(crashlyticsInstance, userId);
...
export const logFailure = async (name: string, reason: string) => {
  ...
  await crashlyticsInstance.recordError(new Error(`Failure: ${name} - ${reason}`));
};
...
export const logUserAction = async (action: string, details?: string) => {
  ...
  await crashlyticsInstance.log(`USER_ACTION: ${action}${details ? ` - ${details}` : ''}`);
};
...
export const setupGlobalErrorHandler = () => {
  ...
  if (typeof process !== 'undefined' && process.on) {
    process.on('uncaughtException', (error) => {
      ...
      crashlyticsInstance.setAttributes({
...
export const setupAxiosInterceptor = (axiosInstance: any) => {
  axiosInstance.interceptors.request.use(
    (config: any) => {
      ...
      crashlyticsInstance.log(`API_REQUEST: ${config.method.toUpperCase()} ${config.url}`);
      if (config.params) {
        const maskedParams = maskSensitiveData(config.params);
        crashlyticsInstance.log(`REQUEST_PARAMS: ${JSON.stringify(maskedParams)}`);
      }
      if (config.data) {
        const maskedData = maskSensitiveData(config.data);
        crashlyticsInstance.log(`REQUEST_BODY: ${JSON.stringify(maskedData)}`);
      }
      return config;
    },
    (error: any) => {
  ...
  axiosInstance.interceptors.response.use(
    (response: any) => {
      ...
      const crashlyticsInstance = getCrashlytics();
      crashlyticsInstance.log(`API_RESPONSE: ${response.config.method.toUpperCase()} ${response.config.url} - ${response.status}`);
      if (response.data) {
        const maskedData = maskSensitiveData(response.data);
        crashlyticsInstance.log(`RESPONSE_DATA: ${JSON.stringify(maskedData)}`);
      }
      return response;
    },
    (error: any) => {
